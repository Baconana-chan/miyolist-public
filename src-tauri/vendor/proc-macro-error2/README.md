...
## ⚠️ Vendored copy — patched for future Rust compatibility

This crate is a **vendored copy of `proc-macro-error2 v2.0.1`** (MIT OR Apache-2.0),
checked into this repository and wired in via `[patch.crates-io]` in `../Cargo.toml`.

Upstream is archived (RUSTSEC-2026-0173) with no patched release. The only
difference from the published crate is a one-line fix in `src/lib.rs`:
...
Without it, a future Rust version rejects the `pub use proc_macro;` re-export
of a private `extern crate` (rust-lang/rust#127909, E0365). If upstream ever
releases a fixed version, delete this directory and the `[patch.crates-io]`
section. Do not edit other files in here.
